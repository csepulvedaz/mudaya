...
const ChatHeader = (props) => {
    ...
    const { loading, data: dataU } = useQuery(PROFILEUSER, {
        variables: { _id: idUser },
    });
    const { loading: loading1, data: dataD } = useQuery(PROFILEDRIVER, {
        ...
        <div className={classes.root}>
            {context.client === "driver" && (
                <>
                    ...
                    <div className={classes.phone}>Celular: {phoneU}</div>
                </>
            )}
            {context.client === "user" && (
                <>
                    ...
                    <div className={classes.phone}>Celular: {phoneD}</div>
                </>
            )}
        </div>
    );
